...
Credenciais de Usuário __(user_[email])__:  Quando uma conta é criada, os dados de autenticação são salvos em uma chave única atrelada ao e-mail.
...
Isolamento de Agendamentos __(agendamentos_[email]):__ As listas de serviços solicitados são salvas em chaves exclusivas para cada usuário. Isso garante que a Tela de Acompanhamento renderize apenas os agendamentos de quem está logado no momento.
